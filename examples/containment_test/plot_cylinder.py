...
points[:, 2] *= 2.0
start = time.time()
contained = containment_test.points_in_cylinder(points, cylinder2origin, radius, length)
stop = time.time()
print(f"{stop - start} s")
ax.scatter(points[::10, 0], points[::10, 1], points[::10, 2], c=contained[::10])
ppu.plot_cylinder(ax=ax, A2B=cylinder2origin, radius=radius, length=length, wireframe=True, color="r")
