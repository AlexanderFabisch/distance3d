...
from distance3d.random import rand_cylinder
from distance3d.colliders import BoundingVolumeHierarchy, Cylinder
...
timer = Timer()
cylinder_frames = list(range(2000))
fast_transform_manager_initialization(tm, cylinder_frames, "base")
for i in cylinder_frames:
    cylinder2origin, radius, length = rand_cylinder(
        random_state, center_scale=2.0, radius_scale=0.1, length_scale=0.5)
    collider = Cylinder(cylinder2origin, radius, length)
    collider.make_artist(c=(0, 1, 0))

    timer.start("aabbtree")
    tm.add_transform(i, "base", cylinder2origin)
